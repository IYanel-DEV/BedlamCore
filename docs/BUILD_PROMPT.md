...
5. Setup is transactional: all changes are held in a per-operator draft. Apply validates every required field, saves the world, persists configuration, and returns the operator to the lobby; Cancel discards edits and deletes a newly-created draft world.
6. The setup flow records a waiting-structure spawn, spectator location, team spawns, beds, forges, item shops, upgrade shops, and diamond/emerald generators. A temporary glass waiting structure exists during waiting/countdown and restores the original blocks when play starts.
...
11. Give operators one contextual setup compass instead of separate menu/setup navigation items. It opens lobby/world setup in the lobby and the current game setup in a game world.
12. Implement balanced teams, countdown, resource generation, bed destruction, respawn while the bed survives, final death, winner detection, and automatic reset.
13. Implement a practical item shop and team upgrades with frozen villager NPCs. Show protected rotating diamond/emerald block markers above their configured generators.
14. Protect the arena: only match-placed blocks and enemy beds may be broken, and placed blocks are removed during reset.
15. Keep lobby and arena configuration in `arenas.yml`; keep transient match and setup-draft state in memory.
