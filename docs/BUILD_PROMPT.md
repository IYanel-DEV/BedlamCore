...
- Use long-lived Bukkit APIs wherever available. Keep the required 1.8 entity-visibility/equipment/silence fallback isolated behind reflection in `compat`/`NpcSoundListener`, with no compile-time NMS dependency, and report the selected path at startup.
...
12. Implement balanced teams, countdown, resource generation, bed destruction without sleep messages or bed-item drops, respawn while the bed survives, final death, winner detection (**contending = living > 0 OR (bed alive AND team was occupied this match)**; never-occupied empty teams do not stall; do not run win-check at match start), and automatic reset. `/leave` and Return-to-Lobby **clear match inventory** then give lobby items only. Match death must skip the vanilla respawn screen (`Player.spigot().respawn()` next tick).
...
14. **Item Shop GUI** uses Hypixel-style Quick Buy categories (Nether Star / Blocks / Melee / Armor / Tools / Ranged / Potions / Utility), lime/gray selection bar, and the Hypixel 21-slot Quick Buy grid/default order. Shift-left-click any category offer, then click a Quick Buy destination; shift-left-click an existing Quick Buy item to remove it. The `BEDLAM_HYPIXEL_API_KEY` environment variable (or `hypixel-api-key` config value) enables a personal `stats.Bedwars.favourites_2` import through the official player endpoint. Offers include wool/clay/glass/endstone/ladder/wood/obsidian/ice; swords+KB stick; armor path; shears + upgradable pick/axe; bow/arrows/punch; **Potions:** Speed II 45s / Jump V 45s / Invis 30s drinkables via PotionMeta+legacy data, emerald costs 1/1/2; apple/snowball/fireball/TNT/pearl/water/milk/sponge, Dream Defender, and Pop-up Tower. Cost lore + cannot-afford red line. **Invis armor hide:** while match INVISIBILITY is active, other players get empty armor equipment packets (hand stays); restore on expire/milk/death/reveal/clear. Modern `sendEquipmentChange`, 1.8 `PacketPlayOutEntityEquipment`. **Tools:** Wood→Stone→Iron→Diamond pick (Efficiency) and axe; shop shows next tier only; death drops 1 tier (min wooden once owned); respawn restores current tier; unbreakable replace-in-inventory. **Upgrades & Traps GUI:** a 45-slot layout with team upgrades (Sharpened / Reinforced / Maniac Miner / Iron Forge / Heal Pool / Cushioned Boots I-II), traps (Blindness / Counter-Offensive / Miner Fatigue / Reveal), and 1/2/4 diamond costs into a 3-slot queue; gray glass separator; trap-queue wool slots; traps fire on base entry with brief cooldown.
...
- Commit only as the human author **IYanel-DEV**. No co-author trailers or third-party authorship lines.
...
- `/bc spawnbuild` rejects glass as cuboid corners (particles only); waiting paste keeps schematic glass.
